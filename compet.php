<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Votre Compte - Sportify</title>
  <link rel="stylesheet" href="styles/sport.css">
  <link href="https://fonts.googleapis.com/css?family=Roboto" rel="stylesheet">
  <link href="https://fonts.googleapis.com/css?family=Poppins:300,400&display=swap" rel="stylesheet">
  <script src="java.js"></script><style>
    header {
  height: 170px;
  }
  nav {
  width: 100%;
  height: 20px;
  }
  </style>
</head>
<body>
  <!-- Contenu de la page de compte -->
  <header>
    <img src="logo.jpg" alt="Logo" class="logo_sportify">
    <nav>
      <ul>
        <li><a href="acceuil.php">Accueil</a></li>
        <li><a class="parcourir-link" href="#">Tout Parcourir</a></li>
        <li><a href="recherche.php">Recherche</a></li>
        <li><a href="rendezvous.php">Rendez-vous</a></li>
        <li><a href="Compte.php">Votre Compte</a></li><?php
// Démarrez la session
      session_start();
      //outils debug objet session : print_r($_SESSION);
// Vérifiez si l'utilisateur est connecté
      if (isset($_SESSION['Email'])) {
    // L'utilisateur est connecté
        ?>
          <li><a href="traitementDeconnexion.php">Déconnexion</a></li>
        <?php
      } else {
    // L'utilisateur n'est pas connecté
      ?>
        <li><a href="connexion.php">Connexion</a></li>
        <li><a href="inscriptionClient.php">Inscription</a></li>
        <?php
        }
        ?>      
        </ul></li>
      </ul>
    </nav>

    <div class="choix">
      <ul>
        <li><a href="activ.php">Activités sportives</a></li>
        <li><a href="compet.php">Les Sports de compétition</a></li>
        <li><a href="salle.php">Salle de sport Omnes</a></li>
      </ul>
    </div>
  </header>



  <main>
    <div class="center-activity">
      <h2>Les Sports de compétition</h2>
    </div>
    <section>
      <div class="activite_box">
        <a href="basketball.php">
          <div class="activity-background">
            <img src="basket.jpg" alt="Flèche">
          </div>
          <span class="activity-title">Basketball</span>
        </a>
      </div>
    </section>
      <div class="activite_box">
        <a href="football.php">
          <div class="activity-background">
            <img src="football.jpg" alt="Chamois">
          </div>
          <span class="activity-title">Football</span>
        </a>
      </div>
      <section>
      <div class="activite_box">
        <a href="rugby.php">
          <div class="activity-background">
            <img src="https://th.bing.com/th/id/OIP.Jq6QbMcjnvKpWjaefAglfQHaEs?w=594&h=376&rs=1&pid=ImgDetMain" alt="Rugby">
          </div>
          <span class="activity-title">Rugby</span>
        </a>
      </div>
    </section>
  <section>
      <div class="activite_box">
        <a href="tennis.php">
          <div class="activity-background">
            <img src="tennis.jpg" alt="Tennis">
          </div>
          <span class="activity-title">Tennis</span>
        </a>
      </div>
      <section>
      <div class="activite_box">
        <a href="natation.php">
          <div class="activity-background">
            <img src="nata.jpg" alt="Natation">
          </div>
          <span class="activity-title">Natation</span>
        </a>
      </div>
      <div class="activite_box">
        <a href="Plongeon.php">
          <div class="activity-background">
            <img src="plongeon.jpg" alt="Plongeon">
          </div>
          <span class="activity-title">Plongeon</span>
        </a>
      </div>
    </section>
  </main>
</body>
</html>

